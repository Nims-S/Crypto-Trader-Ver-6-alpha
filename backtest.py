...
def utc(x):
    return pd.Timestamp(x, tz="UTC") if not isinstance(x, pd.Timestamp) else x.tz_localize("UTC") if x.tzinfo is None else x.tz_convert("UTC")
...
def load_history(symbol: str, timeframe: str, start: str, end: str, cache=True) -> pd.DataFrame:
    ...
    df = df[(df["timestamp"] >= utc(start)) & (df["timestamp"] <= utc(end))].drop_duplicates("timestamp").sort_values("timestamp").reset_index(drop=True)
...
def prep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if compute_indicators:
        ...
        df["bb_upper"] = mid + 2 * std; df["bb_lower"] = mid - 2 * std; df["bb_width"] = (df["bb_upper"] - df["bb_lower"]) / mid
        df["high_20"] = df["high"].rolling(20).max(); df["low_20"] = df["low"].rolling(20).min()
...
def signal(symbol: str, df: pd.DataFrame, p: SignalParams):
    ...
    if reg == "trend" and r["ema20"] > r["ema50"] > r["ema200"] and p.trend_rsi_min <= r["rsi"] <= p.trend_rsi_max and r["vol_ratio"] >= p.trend_vol_min:
        return dict(side="LONG", strategy="trend_follow", regime=reg, confidence=0.75,
                    ...
                    tp1_close_fraction=0.35, tp2_close_fraction=0.50, tp3_close_fraction=0.15,
    ...
    if reg == "breakout" and r["close"] >= r["high_20"] * (1 + p.breakout_close_above_high_pct) and r["rsi"] >= p.breakout_rsi_min and r["vol_ratio"] >= p.breakout_vol_min and r["ema20"] >= r["ema50"]:
        return dict(side="LONG", strategy="breakout_momentum", regime=reg, confidence=0.70,
                    stop_loss_pct=tp(a, p.sl_floor, p.sl_atr_mult * 1.1, p.sl_cap),
                    take_profit_pct=tp(a, p.tp1_floor, p.tp1_atr_mult, p.tp1_cap),
                    secondary_take_profit_pct=tp(a, p.tp2_floor, p.tp2_atr_mult, p.tp2_cap),
                    tp3_pct=tp(a, p.tp3_floor, p.tp3_atr_mult, p.tp3_cap),
                    trail_pct=tp(a, 0.006, p.trail_atr_mult * 1.1, 0.028),
                    tp1_close_fraction=0.35, tp2_close_fraction=0.50, tp3_close_fraction=0.20,
    ...
    if reg == "range" and r["close"] <= r["bb_lower"] * (1 + p.range_touch_pct) and r["rsi"] <= p.range_rsi_max and r["vol_ratio"] <= p.range_vol_max:
        return dict(side="LONG", strategy="range_reversion", regime=reg, confidence=0.60,
                    ...
                    tp3_pct=0.0, trail_pct=tp(a, 0.004, 0.8, 0.018),
                    tp1_close_fraction=0.50, tp2_close_fraction=0.50, tp3_close_fraction=0.0,
    ...
    if reg == "chop" and r["rsi"] <= p.chop_rsi_max and r["close"] > r["ema200"] and r["vol_ratio"] >= p.chop_vol_min:
        return dict(side="LONG", strategy="chop_bounce", regime=reg, confidence=0.55,
                    ...
                    tp3_pct=tp(a, p.tp3_floor, 4.0, p.tp3_cap * 0.75), trail_pct=tp(a, 0.0035, 0.7, 0.015),
                    tp1_close_fraction=0.50, tp2_close_fraction=0.50, tp3_close_fraction=0.10,
                    size_multiplier=p.chop_size_multiplier)
    return None


def slip_rate(bar, cfg):
    vol = max(0.5, min(3.0, float(bar.get("vol_ratio", 1.0) or 1.0)))
    return max(0.0, ((cfg.slippage_bps / 10000.0) + float(bar.get("atr_pct", 0.0) or 0.0) * cfg.slippage_atr_mult) / vol)


def bars_per_year(tf):
    return int((365 * 24 * 60) / int(tf[:-1])) if tf.endswith("m") else int((365 * 24) / int(tf[:-1])) if tf.endswith("h") else 365
...
class Engine:
    def __init__(self, cfg: BacktestConfig, p: SignalParams):
        ...
        self.total_fees = self.total_slip = 0.0

    def _open(self, sym, idx, bar, sig, equity):
        if len(self.positions) >= self.cfg.max_positions or sym in self.positions:
            return
        if calculate_position:
            qty, _ = calculate_position(sym, float(bar["open"]), equity, float(sig["stop_loss_pct"]), float(sig["confidence"]), 1.0, float(sig["size_multiplier"]))
        else:
            qty = equity * 0.33 / float(bar["open"])
        slip = slip_rate(bar, self.cfg)
        price = float(bar["open"]) * (1 + slip)
        fee = price * fee_rate(self.cfg, "taker")
        qty = max(0.0, float(qty))
        if qty <= 0 or fee >= self.cash:
            return
        self.cash -= fee
        self.total_fees += fee
    ...
    def _close(self, sym, bar, pos, price, qty, reason, kind):
        ...
        fill = price * (1 - slip)
        fee = fill * fee_rate(self.cfg, "maker" if kind == "tp" else "taker")
    ...
    def _manage(self, sym, bar):
        ...
        if l <= pos.sl:
            self._close(sym, bar, pos, min(o, pos.sl) if o < pos.sl else pos.sl, pos.qty, "stop_loss", "sl")
            if pos.qty <= 1e-12:
        ...
        for ts in timeline:
            ...
            for s in frames:
                ...
                if s in self.positions:
                    self._manage(s, bar)
            equity = self.cash + sum((prices.get(s, p.high) - p.entry) * p.qty for s, p in self.positions.items())
            self.equity.append((str(ts), equity))
    ...
    def _result(self) -> Result:
        ex = pd.DataFrame([t for t in self.trades if t["type"] == "exit"])
        pnl = ex["pnl"].astype(float) if not ex.empty and "pnl" in ex else pd.Series(dtype=float)
        wins = int((pnl > 0).sum()); losses = int((pnl <= 0).sum())
        gp = float(pnl[pnl > 0].sum()) if len(pnl) else 0.0
        gl = float(abs(pnl[pnl < 0].sum())) if len(pnl) else 0.0
        pf = gp / gl if gl > 0 else float("inf")
        eq = [v for _, v in self.equity] or [self.cfg.initial_capital]
        peak = np.maximum.accumulate(np.array(eq, dtype=float))
        dd = (np.array(eq, dtype=float) - peak) / np.where(peak == 0, 1, peak)
        ret = (eq[-1] / self.cfg.initial_capital - 1) * 100
        returns = np.diff(eq) / np.where(np.array(eq[:-1], dtype=float) == 0, 1, np.array(eq[:-1], dtype=float)) if len(eq) > 1 else np.array([0.0])
        sharpe = (returns.mean() / returns.std(ddof=1) * math.sqrt(bars_per_year(self.cfg.timeframe))) if len(returns) > 1 and returns.std(ddof=1) > 0 else 0.0
        calmar = (ret / 100) / abs(dd.min()) if dd.min() < 0 else 0.0
        return Result(self.p, self.cfg, len(ex), wins, losses, wins / len(pnl) if len(pnl) else 0.0, gp, gl, pf, self.total_fees, self.total_slip, float(pnl.sum()) if len(pnl) else 0.0, eq[-1], ret, float(dd.min() * 100), float(sharpe), float(calmar), float(pnl.mean()) if len(pnl) else 0.0, float(pnl.mean()) if len(pnl) else 0.0, ex.groupby("symbol")["pnl"].sum().astype(float).to_dict() if not ex.empty else {}, ex.groupby("strategy")["pnl"].sum().astype(float).to_dict() if not ex.empty else {}, self.equity, self.trades)


def score(r: Result):
    return r.return_pct * 1.5 + r.win_rate * 100 + r.sharpe * 10 + (15 if math.isfinite(r.profit_factor) else 0) - abs(r.max_drawdown_pct) * 2.5 - r.total_fees / max(1.0, r.config.initial_capital) * 100


def sample_params(rng):
    return replace(SignalParams(), trend_rsi_min=rng.uniform(48, 55), trend_rsi_max=rng.uniform(66, 76), trend_vol_min=rng.uniform(0.95, 1.10), breakout_close_above_high_pct=rng.uniform(0.0015, 0.0050), breakout_rsi_min=rng.uniform(55, 62), breakout_vol_min=rng.uniform(1.20, 1.60), range_bb_width_max=rng.uniform(0.035, 0.055), range_trend_strength_max=rng.uniform(0.0025, 0.0040), range_rsi_max=rng.uniform(26, 34), range_vol_max=rng.uniform(0.95, 1.10), range_touch_pct=rng.uniform(0.0005, 0.0020), chop_rsi_max=rng.uniform(24, 30), chop_vol_min=rng.uniform(0.90, 1.10), sl_atr_mult=rng.uniform(0.8, 1.3), tp1_atr_mult=rng.uniform(1.5, 2.8), tp2_atr_mult=rng.uniform(2.5, 4.5), tp3_atr_mult=rng.uniform(4.5, 7.5), trail_atr_mult=rng.uniform(0.7, 1.5), trend_size_multiplier=rng.uniform(1.0, 1.25), breakout_size_multiplier=rng.uniform(0.85, 1.05), range_size_multiplier=rng.uniform(0.70, 1.0), chop_size_multiplier=rng.uniform(0.55, 0.90))
...
def optimize(data_map, cfg, trials=75, seed=42):
    ...
    for i in range(trials):
        ...
        s = score(r)
        board.append({"trial": i + 1, "score": s, "return_pct": r.return_pct, "max_dd_pct": r.max_drawdown_pct, "win_rate": r.win_rate, "pf": r.profit_factor, "trades": r.total_trades, "params": asdict(p)})
...
def split_df(df, s1, e1, s2, e2):
    df = df.copy(); df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)
    return df[(df["timestamp"] >= utc(s1)) & (df["timestamp"] <= utc(e1))].reset_index(drop=True), df[(df["timestamp"] >= utc(s2)) & (df["timestamp"] <= utc(e2))].reset_index(drop=True)
...
def main():
    ...
    cfg = BacktestConfig(initial_capital=a.capital, maker_fee_bps=a.maker_fee_bps, taker_fee_bps=a.taker_fee_bps, slippage_bps=a.slippage_bps, slippage_atr_mult=a.slippage_atr_mult, latency_bars=a.latency_bars, timeframe=a.timeframe, symbols=tuple(a.symbols), seed=a.seed)
    data = {s: load_history(s, a.timeframe, a.start if a.mode == "backtest" else a.train_start, a.end if a.mode == "backtest" else a.test_end, cache=not a.no_cache) for s in a.symbols}

    if a.mode == "backtest":
        r = Engine(cfg, SignalParams()).run(data)
        print(json.dumps({"total_trades": r.total_trades, "win_rate": r.win_rate, "total_pnl": r.total_pnl, "return_pct": r.return_pct, "max_dd_pct": r.max_drawdown_pct, "sharpe": r.sharpe, "calmar": r.calmar, "pf": r.profit_factor, "fees": r.total_fees, "slippage": r.total_slippage, "by_symbol": r.by_symbol, "by_strategy": r.by_strategy}, indent=2, default=str))
        if a.save:
            (RESULT_DIR / "backtest_result.json").write_text(json.dumps(r.to_dict(), indent=2, default=str))
    elif a.mode == "optimize":
        train = {s: prep(df[(df["timestamp"] >= utc(a.train_start)) & (df["timestamp"] <= utc(a.train_end))].reset_index(drop=True)) for s, df in data.items()}
        p, r, board = optimize(train, cfg, a.trials, a.seed)
        print(json.dumps({"best_params": asdict(p), "train": {"total_trades": r.total_trades, "win_rate": r.win_rate, "total_pnl": r.total_pnl, "return_pct": r.return_pct, "max_dd_pct": r.max_drawdown_pct, "pf": r.profit_factor}}, indent=2, default=str))
        if a.save:
        ...
        print(json.dumps({"best_params": asdict(p), "train": {"total_trades": train_r.total_trades, "win_rate": train_r.win_rate, "total_pnl": train_r.total_pnl, "return_pct": train_r.return_pct, "max_dd_pct": train_r.max_drawdown_pct, "pf": train_r.profit_factor}, "test": {"total_trades": test_r.total_trades, "win_rate": test_r.win_rate, "total_pnl": test_r.total_pnl, "return_pct": test_r.return_pct, "max_dd_pct": test_r.max_drawdown_pct, "pf": test_r.profit_factor}}, indent=2, default=str))
        if a.save:
