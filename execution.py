...
# TP2: close another 33%, then trail the remainder
TP_TIERS = [
    (0.0075, 0.33),  # +0.75%
    (0.0200, 0.33),  # +2.0%
]

# Trailing stop distance for the residual position after TP2 / runner phase
...
def open_position(cur, symbol, price, size, deployed_capital, direction="LONG", atr=None, regime="bull"):
    ...
    cur.execute(
        ...
        INSERT INTO positions
            (symbol, entry, sl, tp, size, regime, confidence, direction, tp1_hit, tp2_hit)
        VALUES
            (%s, %s, %s, %s, %s, %s, %s, %s, FALSE, FALSE)
